    ...
    if (!invoice) {
        return (
            <div className="text-center py-20">
                <h2 className="text-xl font-bold text-gray-600">Sales Invoice not found</h2>
        ...
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-20 font-outfit">
            ...
            <div className="bg-white rounded-[16px] border border-[#E5E7EB] shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between px-8 py-6 border-b border-[#F3F4F6] gap-4">
                    <h2 className="text-[20px] font-bold text-[#111827]">View Sales Invoice</h2>
                    <div className="flex items-center gap-3">
                        {invoice.status !== 'DELETED' && (
                            <button 
                                onClick={() => navigate(`/seller/sales/invoice/edit/${id}`)}
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex gap-2">
                            ...
                            <span className={`px-4 py-1.5 rounded-full text-[13px] font-bold border ${
                                ...
                                {invoice.status === 'DELETED' ? 'DELETED' : 'PUBLISHED'}
                            </span>
                        </div>
                    </div>

                    <div className="border border-[#E5E7EB] rounded-[12px] overflow-hidden shadow-sm">
                        <InfoTableRow label1="Customer Name:" value1={invoice.customerName} label2="Credit Days:" value2={invoice.creditDays} />
                        <InfoTableRow label1="Customer Address:" value1={invoice.address} label2="SO Number:" value2={invoice.soNumber} />
                        <InfoTableRow label1="Invoice No:" value1={invoice.invoiceNumber} label2="Booking Date:" value2={formatDate(invoice.bookingDate)} />
                        <InfoTableRow label1="Invoice Date:" value1={formatDate(invoice.invoiceDate)} label2="GST Number:" value2={invoice.gstNumber} />
                    </div>
                </div>

                {/* Table Section */}
                <div className="p-8">
                    <h3 className="text-[16px] font-bold text-gray-800 mb-4 uppercase tracking-wider underline decoration-emerald-500/30 decoration-4 underline-offset-8">Item Details</h3>
                    <div className="overflow-x-auto custom-invoice-scrollbar border border-[#E5E7EB] rounded-[12px] mb-8 shadow-sm">
                        <table className="w-full min-w-[1200px] border-collapse bg-white">
                            ...
                            <tbody className="divide-y divide-[#F3F4F6]">
                                {items.map((item, index) => (
                                    <tr key={item.id} className="hover:bg-emerald-50/20 transition-colors h-[52px]">
                                        <td className="px-4 py-4 text-center text-[13px] text-[#6B7280] font-medium">{index + 1}</td>
                                        ...
                                        <td className="px-4 py-4 text-[13px] border-l border-[#F3F4F6] text-right font-black">{parseFloat(item.quantity).toFixed(2)}</td>
                                        <td className="px-4 py-4 text-[13px] border-l border-[#F3F4F6] text-center text-gray-500 uppercase font-bold">
                                            {getStandardGstUom(item.uom)}
                                        </td>
                    ...
                    {/* Account Summary */}
                    <h3 className="text-[16px] font-bold text-gray-800 mb-4 uppercase tracking-wider underline decoration-emerald-500/30 decoration-4 underline-offset-8">Account Summary</h3>
                    <div className="border border-[#E5E7EB] rounded-[16px] overflow-hidden shadow-md w-full">
                        <table className="w-full text-left font-outfit">
                            ...
                            <tbody className="divide-y divide-[#F1F5F9]">
                                {/* Material Row */}
                                <tr>
                                    <td className="px-6 py-4 text-[14px] font-bold text-[#475569]">SALES INCOME (EXCL. GST)</td>
                                    ...
                                    <td className="px-6 py-4 text-right font-bold text-[#64748B] border-l border-[#F1F5F9]">₹{materialSubtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                </tr>
                                
                                {/* Direct Expenses (Taxable) */}
                                {(() => {
                                    let runningBalance = materialSubtotal;
                                    return expenses.filter(e => !e.isPostGst).map(exp => {
                                        runningBalance += parseFloat(exp.amount) || 0;
                                        return (
                                            <tr key={exp.id}>
                                                <td className="px-6 py-4 text-[14px] font-medium text-[#64748B] italic">{exp.groupName || 'Direct Income/Charge'}</td>
                                ...
                                {/* GST Rows */}
                                {(() => {
                                    const beforeGstTotal = materialSubtotal + expenses.filter(e => !e.isPostGst).reduce((s, e) => s + (parseFloat(e.amount) || 0), 0);
                                    let currentWithGst = beforeGstTotal;
                                    const rows = [];
                                    
                                    if (invoice.cgstAmount > 0) {
                                        currentWithGst += parseFloat(invoice.cgstAmount);
                                        rows.push(
                                            <tr key="cgst" className="bg-emerald-50/10">
                                                <td className="px-6 py-3 text-[13px] font-bold text-emerald-800 uppercase">C-GST</td>
                                                <td className="px-6 py-3 text-right font-bold text-emerald-800 border-l border-[#F1F5F9]">₹{parseFloat(invoice.cgstAmount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                    ...
                                    if (invoice.sgstAmount > 0) {
                                        currentWithGst += parseFloat(invoice.sgstAmount);
                                        rows.push(
                                            <tr key="sgst" className="bg-emerald-50/10">
                                                <td className="px-6 py-3 text-[13px] font-bold text-emerald-800 uppercase">S-GST</td>
                                                <td className="px-6 py-3 text-right font-bold text-emerald-800 border-l border-[#F1F5F9]">₹{parseFloat(invoice.sgstAmount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                                <td className="px-6 py-3 text-right font-bold text-[#64748b] border-l border-[#F1F5F9]">₹{currentWithGst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                            </tr>
                                        );
                                    }
                                    if (invoice.igstAmount > 0) {
                                        currentWithGst += parseFloat(invoice.igstAmount);
                                        rows.push(
                                            <tr key="igst" className="bg-emerald-50/10">
                                                <td className="px-6 py-3 text-[13px] font-bold text-emerald-800 uppercase">I-GST</td>
                                                <td className="px-6 py-3 text-right font-bold text-emerald-800 border-l border-[#F1F5F9]">₹{parseFloat(invoice.igstAmount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                                <td className="px-6 py-3 text-right font-bold text-[#64748b] border-l border-[#F1F5F9]">₹{currentWithGst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                            </tr>
                                        );
                                    }
                                ...
                                {(() => {
                                    const baseWithGst = materialSubtotal + 
                                                      expenses.filter(e => !e.isPostGst).reduce((s, e) => s + (parseFloat(e.amount) || 0), 0) +
                                                      (parseFloat(invoice.cgstAmount) || 0) + (parseFloat(invoice.sgstAmount) || 0) + (parseFloat(invoice.igstAmount) || 0);
                                ...
                                <tr className="bg-[#073318] text-white">
                                    ...
                                    <td className="px-6 py-5 text-right text-[24px] font-black border-l border-[#ffffff20]">₹{parseFloat(invoice.grandTotal || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
