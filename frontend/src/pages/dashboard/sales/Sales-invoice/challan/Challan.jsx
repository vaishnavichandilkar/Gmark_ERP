                    ...
                    <table className="w-full min-w-[1500px] border-collapse text-left">
                        <thead>
                            <tr className="bg-emerald-900 text-white font-bold text-[14px] uppercase tracking-wider">
                                <th className="px-6 py-5 border-r border-white/10 whitespace-nowrap text-center">{t('common:sr_no', 'SR NO')}</th>
                                ...
                                <th className="px-6 py-5 border-r border-white/10 whitespace-nowrap">{t('modules:customer_type')}</th>
                                ...
                                <th className="px-6 py-5 border-r border-white/10 whitespace-nowrap text-right">{t('modules:taxable_amount_col', 'Taxable Amount')}</th>
                                <th className="px-6 py-5 border-r border-white/10 whitespace-nowrap text-right">{t('modules:tax_amount_col', 'Tax Amount')}</th>
                                <th className="px-6 py-5 border-r border-white/10 whitespace-nowrap text-right">{t('modules:total_amount_col', 'Total Amount')}</th>
                        ...
                        <tbody className={`text-[14px] text-[#111827] font-medium ${isLoading ? 'opacity-40' : 'opacity-100'}`}>
                            {(() => {
                                const pageTaxable = mappedChallans.reduce((sum, r) => sum + (parseFloat(r.taxableAmount) || 0), 0);
                                const pageTax = mappedChallans.reduce((sum, r) => sum + (parseFloat(r.taxAmount) || 0), 0);
                                const pageGross = mappedChallans.reduce((sum, r) => sum + (parseFloat(r.grandTotal) || 0), 0);

                                const grandTaxable = metaGrandTotals ? metaGrandTotals.grandTaxable : challans.reduce((sum, r) => sum + (parseFloat(r.taxableAmount) || 0), 0);
                                const grandGross = metaGrandTotals ? metaGrandTotals.grandTotal : challans.reduce((sum, r) => sum + (parseFloat(r.grandTotal) || 0), 0);
                            ...
                            {mappedChallans.length > 0 ? (
                                mappedChallans.map((row, idx) => (
                                    <tr key={row.id} className="border-b border-[#F3F4F6] hover:bg-[#F9FAFB] transition-all">
                                        ...
                                        <td className="px-6 py-4 font-bold">{row.challanNo}</td>
