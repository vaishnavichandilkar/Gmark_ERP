...
const ViewChallan = () => {
    ...
    if (!challan) {
        return (
            <div className="text-center py-20">
                <h2 className="text-xl font-bold text-gray-600">Challan not found</h2>
    ...
    return (
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-20 font-outfit">
            <style>{`
                .custom-challan-scrollbar::-webkit-scrollbar { height: 6px; }
                .custom-challan-scrollbar::-webkit-scrollbar-track { background: #E5E7EB; }
                .custom-challan-scrollbar::-webkit-scrollbar-thumb { background: #A7C0B8; border-radius: 4px; }
                .custom-challan-scrollbar::-webkit-scrollbar-thumb:hover { background: #014A36; }
            `}</style>
            ...
            <div className="bg-white rounded-[16px] border border-[#E5E7EB] shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden">
                ...
                <div className="p-8 border-b border-[#F3F4F6]">
                    <div className="mb-6">
                        <h1 className="text-[32px] font-bold text-[#111827] mb-2 uppercase tracking-tight">#{challan.challanNumber}</h1>
                    ...
                    <div className="border border-[#E5E7EB] rounded-[12px] overflow-hidden shadow-sm">
                        <InfoTableRow label1="Customer Name:" value1={challan.customerName} label2="Credit Days:" value2={challan.creditDays} />
                        <InfoTableRow label1="Customer Address:" value1={challan.address} label2="SO Number:" value2={challan.soNumber} />
                        <InfoTableRow label1="Challan No:" value1={challan.challanNumber} label2="Booking Date:" value2={formatDate(challan.bookingDate)} />
                        <InfoTableRow label1="Challan Date:" value1={formatDate(challan.challanDate)} label2="GST Number:" value2={challan.gstNumber} />
                    </div>
                </div>

                {/* Table Section */}
                <div className="p-8">
                    <div className="overflow-x-auto custom-challan-scrollbar border border-[#E5E7EB] rounded-[12px]">
                            ...
                            <tbody className="divide-y divide-[#F3F4F6]">
                                {items.map((item, index) => (
                                    <tr key={item.id} className="hover:bg-gray-50/50 transition-colors h-[52px]">
                                        ...
                                        <td className="px-4 py-4 text-[13px] border-l border-[#F3F4F6] text-right font-medium">{parseFloat(item.challanQty || item.quantity).toFixed(2)}</td>
                                        <td className="px-4 py-4 text-[13px] border-l border-[#F3F4F6] text-center text-gray-500 uppercase">
                                        ...
                                        <td className="px-4 py-4 text-[13px] border-l border-[#F3F4F6] text-right font-bold text-emerald-800">
                                            {parseFloat((challan.gstNumber && challan.gstNumber.trim() !== '' && challan.gstNumber !== '-') ? (item.taxPercent || 0) : 0).toFixed(2)}%
                                        </td>
                                ...
                                <tr>
                                    <td colSpan={3} className="px-4 py-5 text-[14px]">Total Summary</td>
                                     <td className="px-4 py-4 border-l border-[#F3F4F6] text-right">
                                        {items.reduce((s, i) => s + (parseFloat(i.challanQty || i.quantity) || 0), 0).toFixed(2)}
                                    </td>
                                    ...
                                    <td className="px-4 py-4 border-l border-[#F3F4F6] text-right text-[#073318]">
                                        ₹{parseFloat(challan.grandTotal || items.reduce((s, i) => s + (parseFloat(i.totalAmount) || 0), 0)).toFixed(2)}
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
                        ...
                        <table className="w-full text-left font-outfit">
                            ...
                            <tbody className="divide-y divide-[#F1F5F9]">
                                ...
                                {(() => {
                                    const materialSubtotal = items.reduce((s, i) => s + (parseFloat(i.beforeTaxAmount) || 0), 0);
                                    let runningBalance = materialSubtotal;
                                    return (challan.expenses || []).filter(e => e.isGstApplicable).map(exp => {
                                ...
                                {(() => {
                                    const materialSubtotal = items.reduce((s, i) => s + (parseFloat(i.beforeTaxAmount) || 0), 0);
                                    const beforeGstTotal = materialSubtotal + (challan.expenses || []).filter(e => e.isGstApplicable).reduce((s, e) => s + (parseFloat(e.amount) || 0), 0);
                                    let currentWithGst = beforeGstTotal;
                                    const rows = [];
                                    
                                    const cgst = parseFloat(challan.cgstAmount || (challan.taxAmount / 2) || 0);
                                    const sgst = parseFloat(challan.sgstAmount || (challan.taxAmount / 2) || 0);
                                    const igst = parseFloat(challan.igstAmount || 0);
                                    ...
                                    if (igst > 0) {
                                        currentWithGst += igst;
                                        rows.push(
                                            <tr key="igst" className="bg-emerald-50/20">
                                                <td className="px-6 py-3 text-[13px] font-bold text-emerald-800 uppercase">I-GST</td>
                                                <td className="px-6 py-3 text-right font-bold text-emerald-800 border-l border-[#F1F5F9]">₹{igst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                                <td className="px-6 py-3 text-right font-bold text-[#64748b] border-l border-[#F1F5F9]">₹{currentWithGst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                            </tr>
                                        );
                                    }
                                ...
                                {(() => {
                                    ...
                                    const baseWithGst = materialSubtotal + 
                                                      (challan.expenses || []).filter(e => e.isGstApplicable).reduce((s, e) => s + (parseFloat(e.amount) || 0), 0) +
                                                      cgst + sgst + igst;
                                    ...
                                    return (challan.expenses || []).filter(e => !e.isGstApplicable).map(exp => {
                                        ...
                                        return (
                                            <tr key={exp.id}>
                                                <td className="px-6 py-4 text-[14px] font-medium text-[#64748B] italic">{exp.groupName || 'Other Charge'}</td>
                                                <td className="px-6 py-4 text-right font-bold text-[#1e293b] border-l border-[#F1F5F9]">₹{parseFloat(exp.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                                <td className="px-6 py-4 text-right font-bold text-[#64748b] border-l border-[#F1F5F9]">₹{runningBalance.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                            </tr>
                                        );
                                ...
                                <tr className="bg-[#073318] text-white">
                                    ...
                                    <td className="px-6 py-5 text-right text-[20px] font-black border-l border-[#ffffff20]">₹{parseFloat(challan.grandTotal || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
