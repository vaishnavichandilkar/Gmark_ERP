...
const PurchaseInvoice = () => {
    ...
    if (currentView === 'add' || currentView === 'edit') {
        return (
            <AddPurchaseInvoice 
                onBack={() => { setCurrentView('list'); setSelectedInvoice(null); }} 
    ...
    return (
        <div className="flex flex-col font-['Plus_Jakarta_Sans'] w-full animate-in fade-in duration-500 px-4 md:px-0">
            {/* Header / Titles matching Account Master */}
            <div className="flex flex-col gap-1 mb-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <h1 className="text-[24px] md:text-[28px] font-bold text-[#111827] tracking-tight">{t('modules:purchase_invoice', 'Purchase Invoice')}</h1>
                    <button 
                        onClick={() => setCurrentView('add')}
                        className="flex items-center justify-center gap-2 bg-[#073318] hover:bg-[#04200f] text-white px-6 h-[44px] rounded-[10px] text-[15px] font-semibold transition-all shadow-sm active:scale-[0.98] w-full sm:w-auto"
                    >
                        <Plus size={18} />
                        {t('modules:add_purchase_invoice', 'Add Purchase Invoice')}
                    </button>
                </div>
                <p className="text-[#6B7280] text-[14px] md:text-[15px]">{t('modules:purchase_invoice_desc', 'View, search, and manage all purchase invoices')}</p>
                
                {/* Embedded Sub-tabs matching previous structure */}
                <div className="flex gap-6 border-b border-[#E5E7EB] w-full max-w-[fit-content] mt-5">
                  {['All', 'Deleted'].map((tab) => (
                    <button
                      key={tab}
            ...
            <div className="bg-white rounded-[16px] border border-[#E5E7EB] shadow-[0_4px_20px_rgba(0,0,0,0.03)] w-full overflow-hidden mb-8">
                ...
                {/* Toolbar */}
                <div className="flex flex-col lg:flex-row lg:items-center justify-between p-4 md:p-6 border-b border-[#F3F4F6] gap-4">
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                        ...
                        <button
                            ...
                            className="flex items-center justify-center w-full sm:w-[42px] h-[42px] border border-[#E5E7EB] text-[#4B5563] rounded-[10px] hover:bg-gray-50 transition-colors bg-white shadow-sm"
                            title="Refresh Data"
                        >
                            <RefreshCw size={18} className="text-gray-400 mr-2 sm:mr-0" />
                            <span className="sm:hidden font-bold">Refresh</span>
                        </button>
                    </div>

                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto" ref={exportRef}>
                        <button 
                ...
                <div className="overflow-x-auto custom-scrollbar relative min-h-[400px]">
                    ...
                    <table className="w-full min-w-[1400px] text-left border-collapse">
                        <thead>
                            <tr className="bg-emerald-900 border-b border-emerald-950 text-[13px] font-bold text-white uppercase tracking-wider">
