...
const DepartmentMaster = () => {
  ...
  // Form State
  const [formData, setFormData] = useState({ prefix: '', departmentName: '' });
  ...
  const fetchDepartments = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_BASE_URL}/masters/department-master`, {
        ...getHeaders(),
        params: { search: searchQuery }
      });
      const deptList = Array.isArray(res.data) ? res.data : (Array.isArray(res.data?.data) ? res.data.data : []);
  ...
  const filteredDepartments = useMemo(() => {
    ...
    if (columnFilters.prefix) {
      ...
      list = list.filter(d => (d.prefix || '').toLowerCase().includes(p));
    }

    if (columnFilters.departmentName) {
      const d = columnFilters.departmentName.toLowerCase().trim();
      list = list.filter(dept => (dept.departmentName || '').toLowerCase().includes(d));
    }

    return list;
  }, [departments, columnFilters]);
  ...
  const handleOpenAdd = () => {
    setFormData({ prefix: '', departmentName: '' });
  ...
  const handleOpenView = (dept) => {
    ...
    setFormData({ prefix: dept.prefix || '', departmentName: dept.departmentName || '' });
    setViewMode('VIEW');
  };
  ...
  const handleSaveDepartment = async (e) => {
    ...
    if (!departmentName) return toast.error('Department Name is required');

    try {
      setActionLoading(true);

      if (viewMode === 'EDIT' && selectedDept) {
        await axios.patch(
          `${API_BASE_URL}/masters/department-master/${selectedDept.id}`, 
        ...
        await axios.post(
          `${API_BASE_URL}/masters/department-master`, 
          { prefix, departmentName }, 
          getHeaders()
        );
  ...
  const handleDeleteExecute = async () => {
    ...
    try {
      setActionLoading(true);
      await axios.delete(`${API_BASE_URL}/masters/department-master/${deptToDelete.id}`, getHeaders());
      toast.success(`Department "${deptToDelete.departmentName}" deleted successfully`);
  ...
  const handleExportExcel = () => {
    if (filteredDepartments.length === 0) return toast.error('No department records to export');
    ...
    XLSX.writeFile(workbook, `Department_Master_${new Date().toISOString().split('T')[0]}.xlsx`);
                  ...
                  {loading ? (
                    ...
                    currentData.map((dept, index) => (
                      <tr key={dept.id} className="hover:bg-[#F9FAFB] transition-colors">
                        ...
                        <td 
                          className={`text-center relative ${activeDropdown === dept.id ? "z-[100]" : ""}`}
                          ref={activeDropdown === dept.id ? dropdownRef : null}
                        >
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
        ...
        <div className="flex flex-col w-full animate-in fade-in duration-300">
          <div className="bg-white rounded-[12px] border border-[#E5E7EB] shadow-sm flex flex-col w-full relative">
            ...
            <div className="p-6 md:p-8">
              <form onSubmit={handleSaveDepartment} className="flex flex-col gap-6 max-w-3xl">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      placeholder="Enter 2-character prefix (e.g. ST)"
